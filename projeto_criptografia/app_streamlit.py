...
from crypto_algorithms.hybrid_module import hybrid_encrypt, hybrid_decrypt
...
algoritmo = st.selectbox("Escolha o algoritmo de criptografia:", ["AES", "DES", "RSA", "Híbrido (RSA + AES)"])
...
if st.button("Criptografar"):
    try:
        if algoritmo == "AES":
            ...
            criptografado = hybrid_encrypt(mensagem, st.session_state["public_key"])
...
if st.button("Descriptografar"):
    try:
        if algoritmo == "AES":
            ...
            texto = hybrid_decrypt(mensagem_cifrada, st.session_state["private_key"])
